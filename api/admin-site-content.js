...
async function upsertContent(pageKey, status, content) {
  ...
  const path = 'huaban_site_content?on_conflict=tenant_id,page_key,status';
  const options = {
    method: 'POST',
    headers: { Prefer: 'resolution=merge-duplicates,return=representation' },
    body: JSON.stringify(payload)
  };
  const rows = await supa(path, options);
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(500).json({ error: error.message || '官网管理保存失败' });
  }
};
